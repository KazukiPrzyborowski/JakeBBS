...
/*
    ...
    $FileInfo: profilemain.php - Last Update: 07/01/1867 Ver 142 Author: Jake $
*/
...
<?php @mysql_free_result($result);
    ...
    if ($_POST['update'] == "now") {
        if ($_POST['act'] == "userinfo" &&
            ...
            if ($OldHashType == "JakeBBSH" && $UpdateHash !== true) {
                $YourPassword = b64e_hmac($_POST['OldPass'], $OldJoined, $OldSalt, "sha1");
                $NewPassword = b64e_hmac($_POST['Password'], $OldJoined, $NewSalt, "sha1");
            }
...
$NewDay = GMTimeStamp();
            ...
            if ($Error != "Yes") {
                ...
                $querynewuserinfo = query("UPDATE `".$Settings['sqltable']."members` SET `Password`='%s',`HashType`='JakeBBSH',`Email`='%s',`LastActive`=%i,`IP`='%s',`Salt`='%s' WHERE `id`=%i", array($NewPassword,$_POST['Email'],$NewDay,$NewIP,$NewSalt,$_SESSION['UserID']));
