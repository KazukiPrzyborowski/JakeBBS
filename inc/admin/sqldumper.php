...
/*
    ...
    $FileInfo: sqldumper.php - Last Update: 07/01/1867 Ver 142 Author: Jake $
*/
...
while ($row = mysql_fetch_row($result)) {
    if (in_array($row[0], $TableChCk)) {
        ...
        $tabstats = mysql_fetch_array($tabsta);
        $AutoIncrement = " ";
...
$renee_s = 0;
echo "-- JakeBBS SQL Dumper\n";
echo "-- version ".$VerInfo['JakeBBS_Ver_SVN']."\n";
echo "-- http://google.com/search?q=JakeBBS+Inc.&btnI=I%27m+Feeling+Luckysupport/\n";
echo "--\n";
...
while ($renee_s < $num) {
    ...
    while ($kazuki_p < $numz) {
        ...
        while ($il < $nums) {
            ...
            if ($kazuki_p === 0) {
                if ($il === 0) {
                    $srow = "INSERT DELAYED IGNORE INTO `".$TableNames[$renee_s]."` (";
                }
